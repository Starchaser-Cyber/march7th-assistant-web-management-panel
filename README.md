...
容器操作区「停止任务（重启容器）」按钮用于终止正在运行的日常 / 差分宇宙 / 货币战争等任务。

**注意**：三月七小助手的任务进程就是容器主进程（PID 1），Linux 下 PID 1 默认忽略 SIGTERM 信号，因此**无法只停任务不停容器**。v1.10 起该按钮通过 `docker compose restart` 重启容器来停止任务，容器重启后如需继续请重新启动任务。
