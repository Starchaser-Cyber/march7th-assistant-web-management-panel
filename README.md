# March7th 小助手网页管理面板 v1.13.1（单文件版）
...
- ✅ 压缩包发布：下载 `m7a_panel_m7a_panel_v1.13.1.zip` 解压后上传到网站目录即可使用，无数据库、无框架依赖（核心仅一个 `index.php`）
- ✅ 手机、电脑浏览器均可使用
...
### 2. 上传并解压文件
把下载的 `m7a_panel_m7a_panel_v1.13.1.zip` 压缩包上传到服务器并解压，将解压出来的**所有文件**（`index.php`、`README.md`、`LICENSE`、`.gitignore`）放到该站点根目录（如 `/www/wwwroot/你的服务器IP_端口/`）。
...
2. 点击「上传」→ 选择本地的 `m7a_panel_m7a_panel_v1.13.1.zip`（支持拖拽上传）
3. 上传完成后，右键该压缩包 → 点击「解压」
4. 解压后确认根目录下有 `index.php` 即为成功（`README.md` 是使用说明、`LICENSE` 是开源许可、`.gitignore` 是 Git 忽略规则，保留即可，不要删除）
5. 资源监控（v1.13+）需要写入 `data/` 目录：解压后如没有 `data/` 目录，请在站点根目录新建并给写权限（宝塔里右键 `data` 目录 → 权限 → 设为 755，属主 www）

**方法二：命令行解压**
```bash
cd /www/wwwroot/你的服务器IP_端口/
unzip m7a_panel_m7a_panel_v1.13.1.zip
ls -l    # 确认 index.php 已在目录中
mkdir -p data && chown -R www:www data && chmod 755 data   # v1.13+ 资源监控采样数据目录
```
