...
- ✅ 压缩包发布：下载 `m7a_panel_v1.3.zip` 解压后上传到网站目录即可使用，无数据库、无框架依赖（核心仅一个 `index.php`）
- ✅ 手机、电脑浏览器均可使用

## 更新记录

### v1.3（2026-08-15）
- 🎨 **全新界面**：仿三月七 WebUI 风格重构，粉色→浅蓝渐变主视觉 + 玻璃拟态卡片
- 📐 **侧边栏布局**：左侧固定导航（概览/任务/配置/日志），告别顶部标签页，更贴近小助手原生 WebUI
...
### 🎨 界面体验（v1.3 全新）
- 🎀 **星铁粉蓝渐变主题（默认）**：粉色→浅蓝渐变主视觉 + 玻璃拟态卡片，三月七 WebUI 同款设计语言
...
### 2. 上传并解压文件
把下载的 `m7a_panel_v1.3.zip` 压缩包上传到服务器并解压，将解压出来的**所有文件**（`index.php`、`README.md`、`LICENSE`、`.gitignore`）放到该站点根目录（如 `/www/wwwroot/你的服务器IP_端口/`）。
...
2. 点击「上传」→ 选择本地的 `m7a_panel_v1.3.zip`（支持拖拽上传）
