...
- ✅ 压缩包发布：下载 `m7a_panel_m7a_panel_v1.6.zip` 解压后上传到网站目录即可使用，无数据库、无框架依赖（核心仅一个 `index.php`）
- ✅ 手机、电脑浏览器均可使用
...
### 2. 上传并解压文件
把下载的 `m7a_panel_m7a_panel_v1.6.zip` 压缩包上传到服务器并解压，将解压出来的**所有文件**（`index.php`、`README.md`、`LICENSE`、`.gitignore`）放到该站点根目录（如 `/www/wwwroot/你的服务器IP_端口/`）。
...
2. 点击「上传」→ 选择本地的 `m7a_panel_m7a_panel_v1.6.zip`（支持拖拽上传）
...
unzip m7a_panel_m7a_panel_v1.6.zip
...
- **手动更新**：打开面板不自动提示，在「更新源」卡片中选择「手动更新」即可切换；需要时点「立即检查」查看是否有新版本

切换入口：概览页 →「更新源」卡片 →「更新模式」下拉框。选择保存在服务器 `.panel_config.php`（自动生成，无需手动编辑）。
...
「更新源」卡片中的「小助手镜像」状态用于判断三月七小助手本体（Docker 镜像）是否落后：
