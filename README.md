...
- ✅ 压缩包发布：下载 `m7a_panel_m7a_panel_v1.12.zip` 解压后上传到网站目录即可使用，无数据库、无框架依赖（核心仅一个 `index.php`）
- ✅ 手机、电脑浏览器均可使用
...
### 2. 上传并解压文件
把下载的 `m7a_panel_m7a_panel_v1.12.zip` 压缩包上传到服务器并解压，将解压出来的**所有文件**（`index.php`、`README.md`、`LICENSE`、`.gitignore`）放到该站点根目录（如 `/www/wwwroot/你的服务器IP_端口/`）。
...
2. 点击「上传」→ 选择本地的 `m7a_panel_m7a_panel_v1.12.zip`（支持拖拽上传）
...
unzip m7a_panel_m7a_panel_v1.12.zip
