...
- ✅ 压缩包发布：下载 `m7a_panel_m7a_panel_v1.13.zip` 解压后上传到网站目录即可使用，无数据库、无框架依赖（核心仅一个 `index.php`）
- ✅ 手机、电脑浏览器均可使用
...
### v1.13（2026-08-22）
- 📈 **新增「资源监控」实时仪表盘**：概览页顶部新增宝塔风格资源卡片——CPU / 内存 / 磁盘 / 系统负载 / 网络速率 / 运行时长六个指标实时跳动，下方三条实时曲线（CPU / 内存 / 磁盘）平滑滚动；打开面板默认每 5 秒采样刷新一次（可在卡片上切换 2~60 秒），CPU 采用双快照差值换算瞬时值，不是平均值，曲线真实反映当下负载
...
- 📦 **新增 data 目录**：采样数据按实例保存在 `data/monitor_容器名.json`（环形保留最近 30 分钟 / 360 点，自动覆盖不膨胀）；请确保网站目录 `data/` 可写
...
### 2. 上传并解压文件
把下载的 `m7a_panel_m7a_panel_v1.13.zip` 压缩包上传到服务器并解压，将解压出来的**所有文件**（`index.php`、`README.md`、`LICENSE`、`.gitignore`）放到该站点根目录（如 `/www/wwwroot/你的服务器IP_端口/`）。
...
2. 点击「上传」→ 选择本地的 `m7a_panel_m7a_panel_v1.13.zip`（支持拖拽上传）
...
unzip m7a_panel_m7a_panel_v1.13.zip
