...
- ✅ 压缩包发布：下载 `m7a_panel_v1.2.zip` 解压后上传到网站目录即可使用，无数据库、无框架依赖（核心仅一个 `index.php`）
- ✅ 手机、电脑浏览器均可使用

## 更新记录

### v1.2（2026-08-14）
- 🖥️ **服务器资源仪表盘**：概览页顶部实时展示 CPU 占用、内存占用、硬盘占用、系统负载、运行时间、网络上下行速率、磁盘读写速率，以及主机名/系统版本
- 📊 **零依赖采集**：纯原生 PHP 读取 /proc 文件系统，无需安装任何监控软件
- 🔄 **自动刷新**：随面板自动刷新同步更新（默认 10 秒），也可手动刷新
...
- 🖥️ **服务器资源仪表盘**（v1.2+）：CPU / 内存 / 硬盘 / 负载 / 网络流量 / 磁盘 IO 实时监控
...
### 2. 上传并解压文件
把下载的 `m7a_panel_v1.2.zip` 压缩包上传到服务器并解压，将解压出来的**所有文件**（`index.php`、`README.md`、`LICENSE`、`.gitignore`）放到该站点根目录（如 `/www/wwwroot/你的服务器IP_端口/`）。

**方法一：宝塔文件管理器（推荐）**
1. 宝塔面板 → 文件 → 进入 `/www/wwwroot/你的服务器IP_端口/`
2. 点击「上传」→ 选择本地的 `m7a_panel_v1.2.zip`（支持拖拽上传）
